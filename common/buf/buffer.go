...
import (
	...
	"github.com/gzjjjfree/hello/common/bytespool"
)
...
// BytesRange 返回此缓冲区的切片，具有给定的起始和结束边界。
func (b *Buffer) BytesRange(from, to int32) []byte {
	if from < 0 {
		from += b.Len()
	}
	// 觉得 from 不能小于0, to 不能大于 len(b.v)
	if to < 0 {
		to += b.Len()
	}
...
func (b *Buffer) ReadFullFrom(reader io.Reader, size int32) (int64, error) {
	...
	b.end += int32(n)
	//fmt.Println("in common-buf-buffer.go func (b *Buffer) ReadFullFrom return read")
	return int64(n), err
}
